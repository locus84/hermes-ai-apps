...
def _artifact_item(path: Path, *, kind: str, url_collection: str | None = None) -> dict[str, Any] | None:
    if not path.is_dir():
        return None
    ...
    item = {
        ...
        "url": f"/dashboard-plugins/{PLUGIN_NAME}/dist/{url_collection or ('apps' if kind == 'app' else 'sessions')}/{guid}/{entry}",
    }
    if kind == "app":
...
def _list_artifacts(base: Path, *, kind: str, url_collection: str | None = None) -> list[dict[str, Any]]:
    if not base.exists():
        return []
    items: list[dict[str, Any]] = []
    paths = sorted(base.iterdir(), key=lambda p: p.stat().st_mtime if p.exists() else 0, reverse=True)
    for path in paths:
        item = _artifact_item(path, kind=kind, url_collection=url_collection)
        if item is not None:
            items.append(item)
    return items
...
async def list_apps() -> dict[str, Any]:
    """Return serverless apps dropped under dist/apps/<guid>/."""
    return {"items": _list_artifacts(APPS_DIR, kind="app")}


@router.get("/archive")
async def list_archive() -> dict[str, Any]:
    """Return apps and legacy static artifacts hidden from the main lists."""
    apps = _list_artifacts(ARCHIVE_APPS_DIR, kind="app", url_collection="archive/apps")
    for item in apps:
...
async def archive_item(collection: str, guid: str) -> dict[str, Any]:
    base, kind, url_collection = _collection_config(collection, archived=False)
...
async def get_app(guid: str) -> dict[str, Any]:
    app_dir = _safe_child_dir(APPS_DIR, guid)
    item = _artifact_item(app_dir, kind="app")
    if item is None:
        raise HTTPException(status_code=404, detail="app not found")
    return item
...
async def call_app_rpc(guid: str, function_name: str, request: Request) -> dict[str, Any]:
    ...
    Apps live under dist/apps/<guid>/ and may define server.py with:
